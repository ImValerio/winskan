...
Winskan is a Go-based tool designed to parse Windows **UserAssist** registry keys as well as **USB Device History**. It extracts and displays information about applications executed on a Windows system, including their run counts, focus times, and the last time they were executed, alongside a detailed history of USB mass storage devices connected to the system.
...
| `--category`   | Select category: `exe` (Executables), `lnk` (Shortcuts), `usb` (USB History), or `all` (default). |
